...
CREATE TABLE Excursiones (
    ...
    fecha DATE NOT NULL CHECK (fecha >= CURRENT_DATE), -- Fecha de la excursión, debe ser futura o presente
    ...
    UNIQUE (descripcion, fecha), -- Restricción única para evitar duplicados en la combinación de descripción y fecha
    CONSTRAINT CHK_precio_pos CHECK (precio_inscripcion >= 0), -- Restricción adicional para asegurar que el precio no sea negativo
    CONSTRAINT CHK_fecha_valida CHECK (fecha >= CURDATE()) -- Asegura que la fecha de la excursión no sea en el pasado
);
...
CREATE TABLE Socios (
    ...
    seguro_contratado ENUM('basico', 'completo') DEFAULT NULL, -- Seguro contratado para los socios estándar (obligatorio para ellos), no aplicable a los federados o infantiles
    ...
    cuota_mensual DECIMAL(10, 2) NOT NULL DEFAULT 10.00, -- Cuota mensual del socio, 10€ por defecto con posibles descuentos aplicables según el tipo de socio
    FOREIGN KEY (numero_padre_o_madre) REFERENCES Socios(numero_socio) -- Relación a la misma tabla, indicando que el padre o madre es un socio existente
        ON DELETE SET NULL ON UPDATE CASCADE, -- Si el padre o madre es eliminado, este valor se pone a NULL
    FOREIGN KEY (federacion_codigo) REFERENCES Federaciones(codigo_federacion) -- Relación con la tabla de federaciones, solo aplica a socios federados
...
CREATE TABLE Federaciones (
    ...
    CONSTRAINT CHK_codigo_federacion CHECK (LENGTH(codigo_federacion) > 0) -- Asegura que el código no esté vacío
);
...
CREATE TABLE Inscripciones (
    ...
    fecha_inscripcion DATE NOT NULL DEFAULT CURRENT_DATE, -- Fecha de inscripción, por defecto es la fecha actual
    FOREIGN KEY (numero_socio) REFERENCES Socios(numero_socio) 
        ON DELETE CASCADE ON UPDATE CASCADE, -- Si se elimina un socio, se eliminan sus inscripciones. Si se actualiza, se actualizan sus inscripciones
    FOREIGN KEY (codigo_excursion) REFERENCES Excursiones(codigo_excursion)
        ON DELETE CASCADE ON UPDATE CASCADE, -- Si se elimina una excursión, se eliminan las inscripciones asociadas. Si se actualiza, también se actualizan sus inscripciones
    CONSTRAINT CHK_fecha_inscripcion CHECK (fecha_inscripcion >= CURDATE()) -- Asegura que la fecha de inscripción no sea en el pasado
);
