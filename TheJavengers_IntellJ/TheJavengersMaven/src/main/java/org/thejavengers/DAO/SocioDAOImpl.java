...
public class SocioDAOImpl implements SocioDAO {
    ...
    public List<Socio> findByExcursionId(int idExcursion) {
        List<Socio> socios = new ArrayList<>();
        String sql = "{CALL obtenerSociosPorExcursion(?)}"; // Procedimiento almacenado

        try (Connection conn = getConnection();
             CallableStatement callableStatement = conn.prepareCall(sql)) {

            callableStatement.setInt(1, idExcursion);
            ...
            try (ResultSet resultSet = callableStatement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    Socio socio = new Socio(idSocio, nombre, apellidos) {
                        @Override
                        ...
                        public float calcularPrecioExcursion(Excursion excursion) {
                            return excursion.getPrecio(); // Lógica predeterminada
                        }
                    };
            ...
            e.printStackTrace();
            throw new RuntimeException("Error al obtener socios inscritos en la excursión: " + e.getMessage(), e);
        }
